    ...
    private static List<String> readLines() {
        try {
            ...
            if (!file.exists()) {
                ...
                FileUtils.copyInputStreamToFile(PrisonPlugin.get().getResource("items.csv"), file);
                return IOUtils.readLines(PrisonPlugin.get().getResource("items.csv"));
            }
            return IOUtils.readLines(Files.newInputStream(file.toPath()));
        } catch (IOException e) {
            e.printStackTrace();
            return new ArrayList<>();
        }
