...
public abstract class PlayerSetting<T> {
    ...
    public void set(Player player, T value) {
        ...
        PrisonPlugin.EXECUTOR.execute(() -> PrisonPlugin.get().getRedisService().executeBackendCommand(redis ->
                redis.hset("playersettings:" + parent + ":" + key,
                        player.getUniqueId().toString(), toString(value))));
    }

    public void load(UUID uuid) {
        values.put(uuid, PrisonPlugin.get().getRedisService().executeBackendCommand(redis -> {
