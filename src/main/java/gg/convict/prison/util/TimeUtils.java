...
public class TimeUtils {
    ...
    public static String formatDate(long input) {
        return formatDate(input, true, Statics.TIME_ZONE, true);
    }

    public static String formatDate(long input, boolean time) {
        return formatDate(input, time, Statics.TIME_ZONE, true);
    }

    public static String formatDate(long input, boolean time, boolean showUtc) {
        return formatDate(input, time, Statics.TIME_ZONE, showUtc);
    }

    public static String formatDate(long input, TimeZone timeZone) {
        return formatDate(input, true, timeZone, true);
    }

    public static String formatDate(long input, boolean time, TimeZone timeZone, boolean showUtc) {
        if (input == -1)
            return "Permanent";

        DateFormat formatter = new SimpleDateFormat("MM/dd/yy" + (time ? " hh:mm:ss a" : "") + (showUtc ? " z" : ""));
